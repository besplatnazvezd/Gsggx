...
# Инструкция для P2P-оплаты (Вы можете изменить реквизиты на свои)
P2P_PAYMENT_INSTRUCTIONS = (
    "1️⃣ *Переведите игровую валюту на реквизиты администрации:*\n"
    "🔗 **Steam Trade Link:** [ТВОЯ_ССЫЛКА_НА_ОБМЕН_STEAM]\n"
    "💳 **QIWI / Карта / Другое:** [ТВОЙ_НОМЕР_QIWI_ИЛИ_КАРТЫ]\n"
    "🎮 **ID / Ник в игре (для прямого обмена):** [ТВОЙ_ИГРОВОЙ_ID_ИЛИ_НИКНЕЙМ]\n"
    "💬 **Связаться с админом напрямую:** tg://user?id={admin_id}\n\n"
    ...
    if len(args) > 1:
        ...
        # ОПЛАТА ЧЕРЕЗ TELEGRAM STARS
            ...
            try:
                ...
                await message.reply("❌ Не удалось сгенерировать чек Stars.")
            return
...
async def process_p2p_user_proof(message: types.Message, state: FSMContext):
    ...
    # Строка уведомления админа (без тройных обратных кавычек для предотвращения багов разметки)
    admin_notification = (
        f"🔔 *НОВАЯ ЗАЯВКА НА P2P ПОДТВЕРЖДЕНИЕ!* (Заявка #{dep_id})\n"
        f"👤 *Пользователь:* @{username} (ID: `{user_id}`)\n\n"
        f"📝 *Подтверждение/Ник/Ссылка от юзера:*\n"
        f"» {proof_text}\n\n"
        f"Проверьте поступление игровой валюты. Для начисления монет NMP используйте админ-команду:\n"
...
async def cmd_give_shortcut(message: types.Message):
    ...
    await message.reply(f"✅ Успешно начислено **{amount:.2f} NMP** пользователю `{target_uid}`!")
    try:
