...
# Глобальный разделитель (полностью исключает баг с умножением строк)
DIVIDER = "═" * 30
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_chat.type in ["group", "supergroup"]:
        await update.message.reply_text(text, reply_markup=get_group_reply_keyboard(), parse_mode="HTML")
    else:
...
async def games_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        f"<b>🎮 ИГРОВОЙ СЕКТОР mCoin</b>\n"
        f"{DIVIDER}\n"
        f"Выберите игру для проверки удачи. \n\n"
        f"⚠️ <i>Правило: принять участие в любой игре можно строго раз в 10 минут!</i>\n"
        f"🎈 <i>В играх нет ставок, вы ничего не теряете!</i>"
    )
...
async def inline_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Проверка приветственных реф-подписок
        ...
        if is_subscribed:
            ...
            await query.message.reply_text("❌ Ошибка: вы подписались не на все каналы. Проверьте подписки и попробуйте еще раз.", parse_mode="HTML")
        return

    # Проверка подписок на спонсоров за mCoin с точечной ошибкой выдачи
        ...
        if is_subscribed:
            ...
            # Твоя точечная ошибка выдачи по ТЗ
            error_text = (
    ...
    # Запуск выбора спонсоров для Дартса (1-10)
        ...
        await query.edit_message_text("🎯 <b>Сделайте ваш прогноз:</b> куда именно прилетит дротик?", reply_markup=keyboard, parse_mode="HTML")
        return
    ...
    # Запуск выбора спонсоров для Кубика (1-10)
        ...
        await query.edit_message_text("🎲 <b>Сделайте ваш прогноз:</b> какая грань выпадет на кубике?", reply_markup=keyboard, parse_mode="HTML")
        return
    ...
    # Запуск выбора спонсоров для Баскетбола (1-10)
        ...
        await query.edit_message_text("🏀 <b>Сделайте ваш прогноз:</b> попадет ли мяч в кольцо?", reply_markup=keyboard, parse_mode="HTML")
        return
...
async def handle_group_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 2. Переход к играм
        ...
        text_games = (
            ...
            f"{DIVIDER}\n"
            f"Выберите игру для проверки удачи. \n\n"
            f"⚠️ <i>Принять участие в игре можно раз в 10 минут! В играх нет ставок, вы ничего не теряете!</i>"
        )
...
async def play_darts(query, context, prediction: str, count: int):
    user_id = query.from_user.id
    await query.message.reply_text("🎯 Бросаем дротик...", parse_mode="HTML")
    ...
    if is_win:
        ...
        await query.message.reply_text(f"🎯 <b>ПОБЕДА!</b>\n\nВыпало: {actual}. Начислено: +{reward:,} mCoin!", reply_markup=get_back_keyboard(), parse_mode="HTML")
    else:
        await query.message.reply_text(f"💨 <b>ПРОМАХ!</b>\n\nВыпало: {actual}, ваш прогноз: {prediction}. Ничего страшного, попробуйте еще раз!", reply_markup=get_back_keyboard(), parse_mode="HTML")

async def play_dice(query, context, prediction: int, count: int):
    user_id = query.from_user.id
    await query.message.reply_text("🎲 Бросаем кубик...", parse_mode="HTML")
    ...
    if is_win:
        ...
        await query.message.reply_text(f"🎲 <b>УГАДАЛИ!</b>\n\nВыпало число: {val}. Начислено: +{reward:,} mCoin!", reply_markup=get_back_keyboard(), parse_mode="HTML")
    else:
        await query.message.reply_text(f"😢 <b>НЕ УГАДАЛИ!</b>\n\nВыпало число: {val}, ваш выбор: {prediction}. Ничего страшного, баланс сохранен!", reply_markup=get_back_keyboard(), parse_mode="HTML")

async def play_basketball(query, context, prediction: str, count: int):
    user_id = query.from_user.id
    stake = count * REWARD_GAME
    
    await query.message.reply_text("🏀 Выполняем бросок мяча в кольцо...", parse_mode="HTML")
...
async def enable_in_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type in ["group", "supergroup"]:
        with sqlite3.connect(DB_FILE) as conn:
            ...
            conn.commit()
        await update.message.reply_text("✅ Бот успешно активирован для данной группы!", reply_markup=get_group_reply_keyboard(), parse_mode="HTML")

async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["state"] = None
    await update.message.reply_text("Ввод успешно сброшен. Воспользуйтесь меню:", reply_markup=get_main_inline_keyboard(), parse_mode="HTML")
...
async def scheduled_loop(app: Application):
    ...
    while True:
        try:
            ...
            for group in groups:
                try:
                    await app.bot.send_message(chat_id=group[0], text=text, reply_markup=keyboard, parse_mode="HTML")
                except Exception as e:
