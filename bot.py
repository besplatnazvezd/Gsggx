...
def db_init():
    ...
    with sqlite3.connect(DB_FILE) as conn:
        ...
        # Попытка добавить новые колонки для совместимости со старыми версиями БД
        try:
...
def db_add_reward(tg_id: int, amount: int, description: str):
    """Безопасное начисление или списание средств с записью в историю."""
    with sqlite3.connect(DB_FILE) as conn:
        cursor = conn.cursor()
...
async def get_piarflow_sponsors(count: int) -> list:
    """Запрос к API PiarFlow для получения списка активных каналов."""
    try:
        async with httpx.AsyncClient() as client:
            # Реальный запрос к API (активируется при установке API_KEY)
    ...
    # Резервные каналы, если API не настроено
    return [
...
async def check_piarflow_subscriptions(user_id: int, sponsors: list) -> bool:
    ...
    return True # Резервное значение успеха
...
def get_main_inline_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        # Яркая, побуждающая к действию кнопка для заработка
        [InlineKeyboardButton("🤑 НАЖМИ СЮДА, ЧТОБЫ ПОЛУЧИТЬ mCoin 🤑", callback_data="menu_get_mcoin")],
        [
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Красивая текстовая разделительная линия (без багов умножения строк!)
    divider = "═" * 30
        
    text = (
        f"Приветствую! Тут ты можешь получить mCoin за подписку на спонсоров, "
        f"совершенно бесплатно и без вложений!\n\n"
        f"📢 Канал — @укажу\n"
        f"💬 Чат — @Укажу\n"
        f"🏆 Выдачи — @mCoin_REP\n\n"
        f"{divider}\n"
...
async def games_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Вызов списка игр напрямую текстом или по команде /games."""
    divider = "═" * 30
    text = (
...
async def inline_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
            ...
            if referrer_id and user_data.get("ref_reward_paid") == 0:
                ...
                try:
                    await context.bot.send_message(
                        chat_id=referrer_id,
                        text=f"🎁 <b>Реферальный бонус!</b>\nВаш реферал успешно прошел модерацию. Вам начислено +{REFERRAL_REWARD:,} mCoin."
                    )
                except Exception:
                    pass
    ...
    # Запуски игр из Инлайн-панели
    if data == "game_darts":
        context.user_data["state"] = "awaiting_darts_sponsors"
        await query.edit_message_text("🎯 <b>Игра ДАРТС</b>\n\nУкажите число спонсоров, на которых вы готовы подписаться для входа в игру:")
    ...
    # Результаты прогнозов
        ...
        prediction = parts[2] # выбранная ставка/вариант
        sponsors_count = context.user_data.get("game_sponsors_count", 1)
        
        # Обновляем время последней игры (кулдаун)
...
# ---------------------------------------------------------------------------
# ОБРАБОТЧИК ТЕКСТОВОГО ВВОДА (STATE MACHINE)
# ---------------------------------------------------------------------------
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Поддержка команды сброса /cancel во время ввода
    if text == "/cancel":
        context.user_data["state"] = None
        await update.message.reply_text("❌ Ввод успешно отменен. Вы возвращены на главный экран.", reply_markup=get_main_inline_keyboard())
        return
    ...
    if state == "awaiting_mcoin_sponsors":
        try:
            ...
            await update.message.reply_text("Введите корректное число спонсоров (целое, больше 0) или напишите /cancel.")
            return
    ...
    # Дартс спонсоры
        ...
        try:
            ...
            await update.message.reply_text("Введите число спонсоров (больше 0).")
            return
        ...
        await update.message.reply_text("🎯 <b>Сделайте ваш прогноз:</b> куда именно прилетит дротик?", reply_markup=keyboard)
    ...
    # Кубик спонсоры
        ...
        await update.message.reply_text("🎲 <b>Сделайте ваш прогноз:</b> какая грань выпадет на кубике?", reply_markup=keyboard)
    ...
    # Баскетбол спонсоры
        ...
        try:
            ...
            await update.message.reply_text("Введите число спонсоров.")
            return
        ...
        if user_db["balance"] < required_stake:
            await update.message.reply_text(
                f"❌ <b>Ошибка: Недостаточный баланс!</b>\n\n"
                f"Ваша ставка на игру: <code>{required_stake:,} mC</code>.\n"
                f"Ваш текущий баланс: <code>{user_db['balance']:,} mC</code>.\n\n"
                f"Укажите меньшее число спонсоров или напишите /cancel."
            )
            return
        ...
        await update.message.reply_text("🏀 <b>Сделайте ваш прогноз:</b> попадет ли мяч в кольцо?", reply_markup=keyboard)

# ---------------------------------------------------------------------------
# РАСЧЕТ РЕЗУЛЬТАТОВ ИГР (С ЭМОДЗИ)
# ---------------------------------------------------------------------------
async def play_darts(query, context, prediction: str, count: int):
    ...
    if is_win:
        ...
        await query.message.reply_text(f"💨 <b>ПРОМАХ!</b>\n\nВыпало: {actual}, ваш прогноз: {prediction}.", reply_markup=get_back_keyboard())
...
async def play_dice(query, context, prediction: int, count: int):
    ...
    if is_win:
        ...
        await query.message.reply_text(f"😢 <b>НЕ УГАДАЛИ!</b>\n\nВыпало число: {val}, ваш выбор: {prediction}.", reply_markup=get_back_keyboard())
...
async def play_basketball(query, context, prediction: str, count: int):
    ...
    if is_win:
        mult = 2.4 if prediction == "Mимо" else 1.6
        ...
        db_add_reward(user_id, -stake, "Слив в Баскетбол")
        await query.message.reply_text(
            f"💨 <b>МИМО КОЛЬЦА!</b>\n\n"
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["state"] = None
    await update.message.reply_text("Ввод сброшен. Воспользуйтесь меню управления:", reply_markup=get_main_inline_keyboard())
